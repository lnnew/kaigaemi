module.exports = {
  HTML:function(user, year,jo_info,jangos,ipchals,ranking){
    ...
    <html>
      ...
      var quantities=[{
                    ...
                    function refresh() {
                      ...
                      for (let i=1;i<4;i++){
                        $('#jong'+i).text(Math.floor(year_budget*0.04*i));

                      }

                      $("#hint4").val(Math.floor(year_budget*0.04*1));
                      $("#hint20").val(Math.floor(year_budget*0.04*2));
                      $("#hint8").val(Math.floor(year_budget*0.04*2));
                      $("#hint12").val(Math.floor(year_budget*0.12));
                      $('#po_hint').text(Math.floor(year_budget*0.08));
    ...
    <p id ="gyul" style="margin-left:30px">${year-1}년 결과</p>
        <table class="deung" >
      <tr>
        <th>등수</th>
        <th>조</th>
