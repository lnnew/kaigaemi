...
//세무조사
 if (year==2019){
   ...
   for (let i =0; i<3; i++){
     await pool.query("UPDATE jo SET year_budget = year_budget-50 WHERE jo=$1",[semu_ranking[i]['jo'] ])
      await pool.query("UPDATE jo SET budget = budget-50 WHERE jo=$1",[semu_ranking[i]['jo']  ])
       await pool.query("UPDATE jo SET cash = cash-50 WHERE jo=$1",[semu_ranking[i]['jo']  ])
   }
 } else if (year ==2021){
   let semu_ranking = await pool.query("SELECT jo FROM jo ORDER BY year_budget DESC",[]);
   semu_ranking = semu_ranking.rows;
   console.log("semu",semu_ranking)
   for (let i =0; i<3; i++){
     await pool.query("UPDATE jo SET year_budget = year_budget-100 WHERE jo=$1",[semu_ranking[i]['jo'] ])
      await pool.query("UPDATE jo SET budget = budget-100 WHERE jo=$1",[semu_ranking[i]['jo'] ])
       await pool.query("UPDATE jo SET cash = cash-100 WHERE jo=$1",[semu_ranking[i]['jo'] ])
   }
 }
